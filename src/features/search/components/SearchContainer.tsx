  ...
  return (
    <>
      <AnimatePresence mode="wait">
        {!hasSearched && (
          <motion.div initial={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0, marginBottom: 0 }} transition={{ duration: 0.3, ease: 'easeInOut' }}>
            {featureCard}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <motion.div layout transition={{ type: 'spring', bounce: 0.2, duration: 0.6 }} className="relative min-h-[100px]">
        <AnimatePresence mode="wait">
          {isLoading && (
            <motion.div
              key="loading"
              ...
              className="absolute inset-0 mt-6 flex justify-center"
            >
              <div className="relative max-w-md bg-gray-800/20 backdrop-blur-sm rounded-lg border border-white/[0.08] shadow-lg overflow-hidden">
                ...
                <div className="relative px-4 py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col gap-0.5">
                      <span className="text-sm font-medium bg-linear-to-r/oklch from-blue-400/90 to-blue-300/90 text-transparent bg-clip-text">
                        Analyzing Posts
                        {profileData?.postsCount !== undefined && ` (${profileData.postsCount.toLocaleString()} total)`}
                      </span>
                      <span className="text-sm text-gray-400">Finding the most engaging posts</span>
                    </div>
                  </div>
                </div>
              </div>
